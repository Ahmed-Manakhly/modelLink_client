...
import { Button, Form, Table, Tabs, Tab, Alert } from 'react-bootstrap';
    ...
    return (
        <div>
            <div className="d-flex justify-content-between align-items-center mb-3">
                <div>
                    <h5 className="mb-1">Taxonomy Management</h5>
                    <p className="text-muted small mb-0">Categories (parent → children), modalities, and body parts used when publishing models.</p>
                </div>
            ...
            <Tabs activeKey={activeTab} onSelect={(k) => setActiveTab(k || 'categories')} className="mb-3">
                <Tab eventKey="categories" title="Categories">
                    <div className="d-flex justify-content-end mb-2">
                        <Button size="sm" onClick={() => openCategoryForm('create')}>Add parent group</Button>
                    </div>
                    <Table bordered hover responsive size="sm" className="bg-white">
                        ...
                        <tbody>
                            {categories.length === 0 && (
                                <tr><td colSpan={5} className="text-muted text-center py-4">No categories yet. Add a parent group or run taxonomy seeding.</td></tr>
                            )}
                                ...
                                <React.Fragment key={parent.id}>
                                    <tr className="table-light">
                                        <td>
                                            ...
                                            <div className="text-muted small">{parent.slug}</div>
                                        </td>
                                        <td>Parent group</td>
                                        ...
                                        <td>
                                            <Button size="sm" variant="outline-primary" className="me-1" onClick={() => openCategoryForm('edit', parent)}>Edit</Button>
                                            <Button size="sm" variant="outline-success" className="me-1" onClick={() => openCategoryForm('create', null, parent.id)}>Add child</Button>
                                            <Button size="sm" variant="outline-danger" onClick={() => openDelete('category', parent)}>Delete</Button>
                                        </td>
                                    </tr>
                                    {(parent.children || []).map((child) => (
                                        <tr key={child.id}>
                                            <td className="ps-4">↳ {child.name}<div className="text-muted small">{child.slug}</div></td>
                                            <td>Subcategory</td>
                                            <td>{child._count?.models || 0} models</td>
                                            <td>—</td>
                                            <td>
                                                <Button size="sm" variant="outline-primary" className="me-1" onClick={() => openCategoryForm('edit', { ...child, parentId: parent.id })}>Edit</Button>
                                                <Button size="sm" variant="outline-danger" onClick={() => openDelete('category', child)}>Delete</Button>
                                            </td>
                                        </tr>
                                    ))}
                                </React.Fragment>
                ...
                <Tab eventKey="modalities" title="Modalities">
                    <div className="d-flex justify-content-end mb-2">
                        <Button size="sm" onClick={() => openSimpleForm('modality', 'create')}>Add modality</Button>
                    </div>
                    <SimpleTaxonomyTable
                        rows={modalities}
                        countKey="versions"
                ...
                <Tab eventKey="bodyparts" title="Body parts">
                    <div className="d-flex justify-content-end mb-2">
                        <Button size="sm" onClick={() => openSimpleForm('bodypart', 'create')}>Add body part</Button>
                    </div>
                    <SimpleTaxonomyTable
                        rows={bodyParts}
                        countKey="versions"
            ...
            {formModal && (
                <Modal onClose={() => setFormModal(null)}>
                    <Form onSubmit={submitForm} className="p-4">
                        <h5 className="mb-3">{formModal.mode === 'create' ? 'Create' : 'Edit'} {formModal.type}</h5>
                        ...
                        {formModal.type === 'category' && (
                            <>
                                ...
                                <Form.Group className="mb-3">
                                    <Form.Label>Icon (optional, parent groups only)</Form.Label>
                                    <Form.Control
                                        type="file"
                                        accept="image/*"
                        ...
                        <div className="d-flex justify-content-end gap-2">
                            <Button variant="secondary" type="button" onClick={() => setFormModal(null)}>Cancel</Button>
            ...
            {deleteModal && (
                <Modal onClose={() => setDeleteModal(null)}>
                    <div className="p-4">
                        <h5 className="mb-3">Delete {deleteModal.item.name}?</h5>
                        {deleteModal.type === 'category' && (
                            <>
                                ...
                                {deleteModal.impact.childCount > 0 && (
                                    <Form.Group className="mb-3">
                                        <Form.Label>Reassign subcategories to</Form.Label>
                                        <Form.Select
                                ...
                                {deleteModal.impact.modelCount > 0 && (
                                    <Form.Group className="mb-3">
                                        <Form.Label>Reassign models to</Form.Label>
                                        <Form.Select
                        ...
                        {(deleteModal.type === 'modality' || deleteModal.type === 'bodypart') && deleteModal.impact.versionCount > 0 && (
                            <>
                                ...
                                <Form.Group className="mb-3">
                                    <Form.Label>Reassign versions to</Form.Label>
                                    <Form.Select
                        ...
                        {(deleteModal.impact.childCount === 0 && deleteModal.impact.modelCount === 0 && deleteModal.impact.versionCount === 0) && (
                            <p className="text-muted">Nothing is linked to this entry. Safe to delete.</p>
                        )}
                        <div className="d-flex justify-content-end gap-2">
...
function SimpleTaxonomyTable({ rows, countKey, onEdit, onDelete }) {
    return (
        <Table bordered hover responsive size="sm" className="bg-white">
            ...
            <tbody>
                {rows.length === 0 && (
                    <tr><td colSpan={4} className="text-muted text-center py-4">No entries yet.</td></tr>
                )}
                {rows.map((row) => (
                    <tr key={row.id}>
                        <td>{row.name}</td>
                        ...
                        <td>
                            <Button size="sm" variant="outline-primary" className="me-1" onClick={() => onEdit(row)}>Edit</Button>
                            <Button size="sm" variant="outline-danger" onClick={() => onDelete(row)}>Delete</Button>
                        </td>
                    </tr>
                ))}
            </tbody>
